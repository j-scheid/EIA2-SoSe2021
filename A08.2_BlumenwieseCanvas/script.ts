...
namespace BlumenwieseCanvas {
  ...
  function drawGround(_position: Vector): void {
  ...
  function drawBeehive(_position: Vector): void {
    ctx.beginPath();
  ...
  function drawMountains(
    ...
    let gradient: CanvasGradient = ctx.createLinearGradient(0, 0, 0, -max);
  ...
  function drawFlower(_color: string): void {
    var placeTulipsX: number = Math.random() * canvas.width - 1;
    var placeTulipsY: number =
      Math.random() * (canvas.height - canvas.height * 0.6) +
      canvas.height * 0.6;
